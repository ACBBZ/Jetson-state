...
class Analyser:
	...
	def model_report(self):
		...
		def power_consumption():
			tempLoadFile2="/sys/bus/i2c/drivers/ina3221x/1-0040/iio:device0/in_power0_input"#GPU POWER
			with open(tempLoadFile2,'r') as tempFile2:
				temp2=tempFile2.read()

			tempLoadFile3="/sys/bus/i2c/drivers/ina3221x/1-0040/iio:device0/in_power1_input"#CPU POWER
			with open(tempLoadFile3,'r') as tempFile3:
				temp3=tempFile3.read()

			tempLoadFile4="/sys/bus/i2c/drivers/ina3221x/1-0040/iio:device0/in_power2_input"#SOC POWER
			with open(tempLoadFile4,'r') as tempFile4:
				temp4=tempFile4.read()

			tempLoadFile5="/sys/bus/i2c/drivers/ina3221x/1-0041/iio:device1/in_power0_input"#CV POWER
			with open(tempLoadFile5,'r') as tempFile5:
				temp5=tempFile5.read()

			tempLoadFile6="/sys/bus/i2c/drivers/ina3221x/1-0041/iio:device1/in_power1_input"#VDDRQ POWER
			with open(tempLoadFile6,'r') as tempFile6:
				temp6=tempFile6.read()

			tempLoadFile7="/sys/bus/i2c/drivers/ina3221x/1-0041/iio:device1/in_power2_input"#SYS5V POWER
			with open(tempLoadFile7,'r') as tempFile7:
				temp7=tempFile7.read()

			power_value=float(temp2)+float(temp3)+float(temp4)+float(temp5)+float(temp6)+float(temp7)
			return power_value
		...
		plt.plot(timer, cpu_list,label='CPU Usage')	
		plt.title('Model Inference Report')
		plt.ylabel('Usage (%)')
		plt.title('AVG GPU usage: %f , Avg CPU Used: %f'%(statistics.mean(gpu_list),statistics.mean(cpu_list)))
		plt.legend()
		...
		plt.savefig('/home/nvidia/data/model.png')
